...
public class SelectUserResponse extends Response {
  ...
  static {
    ...
    for (int i = 0; i < FIELD_COUNT; ++i) {
      fields[i] = PacketUtils.getField(packetId++,
          "USER()", Fields.FIELD_TYPE_VAR_STRING);
    }
  ...
  public void response() {
    ...
    sb.append(ss.user).append('@').append(ss.host);
    row.add(StringUtils.encodeString(sb.toString(), ss.charset));
